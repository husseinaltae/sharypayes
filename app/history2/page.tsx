...
export default function PaymentHistory() {
  ...
  useEffect(() => {
    ...
    if (searchOffice.trim()) {
      const allRelatedOfficeIds = getDescendantOfficeIds(searchOffice);
      const matchedEmpIds = employees
        .filter(e => allRelatedOfficeIds.includes(e.office_id))
        .map(e => e.id);
      filteredData = filteredData.filter(p => matchedEmpIds.includes(p.employee_id));
    }

    setFiltered(filteredData);
  }, [searchName, searchMonth, searchOffice, payments, employees, offices]);
